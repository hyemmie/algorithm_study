...
bool isDominated(int x, int y)
{
    ...
    // y는 새로운 점의 y좌표, it->second는 가장 왼쪽에 있는 점의 value(key가 first, value가 second)
    return y < it->second;
}
...
void removeDominated(int x, int y)
{
    ...
    if (it == coords.begin())
        return;
    // lower.bound()는 오른쪽 중 가장 왼쪽이므로 하나 빼면 x,y자체. 거기서 시작?
    --it;
    // 루프 불변 조건: it 는 (x,y) 보다 바로 왼쪽에 있는 점
    while (true)
    {
        // (x,y) 바로 왼쪽에 오는 점을 찾는다
        // it 가 표시하는 점이 (x,y) 에 지배되지 않는다면 곧장 종료 (일종의 기저 사례!!!)
        // 왼쪽에 있는 점의 y좌표가 더 크면! 지배당하지 않으니 종료
        if (it->second > y)
            break;
